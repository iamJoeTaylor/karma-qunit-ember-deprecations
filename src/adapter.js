function createQUnitConfig (karma, defaultConfig) {
      ...
      if (window.Ember) {
        if (Ember.Debug && Ember.Debug.registerDeprecationHandler) {
          Ember.Debug.registerDeprecationHandler(function(message, options, next) {
            ...
            testResult.errors.push('DEPRECATION: ' + message + '\n' + (new Error().stack));

            next(message, options);
          });
        } else {
          var originalDeprecate = window.Ember.deprecate;
          window.Ember.deprecate = function(message, test) {
            if (test) { return; }

            testResult.success = false;
            testResult.errors.push('DEPRECATION: ' + message + '\n' + (new Error().stack));

            originalDeprecate(message, test);
          };
        }
      }
